...
using static SimpleInventory.BL.BusinessRepoIIExt;

//using static SimpleInventory.BL.BusinessRepoExt;
namespace SimpleInventoryTest
{
    public class BusinessRepoIITest
    {
        public class Person
        {
        ...
        public void Given_A_BusinessRepo_Of_T_Bind_To_Get_BusinessRepo_Of_R()
        {
            ...
            BusinessRepoII<Person> personRepo = p;
            var StudentRepo = personRepo.Bind(x =>
                                                  x.Select(y =>
                                                              y.Match(
                                                                  () => None,
                                                                  r => Some(new Student
                                                                  {
                                                                      StudentID = $"{r.Id}-{r.Name}",
                                                                      Name = r.Name
                                                                  })
                                                                  )).ToBusinessRepoII());

            //Assert.Equals($"{p.Id}-{p.Name}",StudentRepo.Data.AsEnumerable())
            StudentRepo.Data.First().Match(
                ()=>Assert.False(true),
                val=>Assert.Equal($"{p.Id}-{p.Name}",val.StudentID)
                );
        }
        ...
        //{
        //    BusinessRepoII<Person> peopleRepo = lst.Select(x=>Some(x)).ToBusinessRepoII();
        ...
        //                                    )
        //                                ).ToList());
        //    var a = StudentRepo.Data.Values.Select(x=>x.Name).ToList();
